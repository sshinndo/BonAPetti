...
import 'package:pet_service_application/community/screen/PostScreen.dart';
...
class _CommunityWidget extends State<CommunityWidget> {
  ...
  Widget build(BuildContext context) {
    ...
    const double nameSize = 18; //이름 글자 크기
    ...
    return GestureDetector(
        ...
        Navigator.push(
            context,
            MaterialPageRoute(
                builder: (context) => PostScreen(communityInfo: widget.communityInfo)));  //터치시 포스트 상세 창으로 이동
      },
        child: Card(
            elevation: 0,
                ...
                Container(
                  ...
                  child:
                      ...
                      widget.communityInfo.imageUrls.isEmpty
                          ...
                          ? SizedBox(height: 50,)
                          //첫 이미지 (대표 이미지)
                          : Image.network(widget.communityInfo.imageUrls.first,
                        fit: BoxFit.fill,
...
class CommunityIcon extends StatefulWidget {
  final CommunityInfo communityInfo; //굿즈 정보 객체

  CommunityIcon({Key? key, required this.communityInfo})
      : super(key: key); //생성자

  @override
  _CommunityIcon createState() => _CommunityIcon(); //위젯 생성
}

class _CommunityIcon extends State<CommunityIcon> {
